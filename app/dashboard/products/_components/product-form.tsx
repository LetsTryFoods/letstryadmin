...
import { productFormSchema, ProductFormValues, getDefaultVariant } from "@/lib/validations/product"
  ...
  useEffect(() => {
    watchedVariants?.forEach((variant, index) => {
      ...
      if (mrp > 0 && price > 0 && price <= mrp) {
        ...
        const currentDiscount = form.getValues(`variants.${index}.discountPercent`)
        if (currentDiscount !== discount) {
          form.setValue(`variants.${index}.discountPercent`, discount, { shouldValidate: false })
        }
      } else if (mrp > 0 && price === mrp) {
        const currentDiscount = form.getValues(`variants.${index}.discountPercent`)
        if (currentDiscount !== 0) {
          form.setValue(`variants.${index}.discountPercent`, 0, { shouldValidate: false })
        }
      }
    })
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      const formattedVariants = data.variants.map(variant => {
        const formattedImages = variant.images
          ?.filter(img => img.preview || img.finalUrl || img.url)
          .map(img => ({
            url: img.finalUrl || img.url || img.preview || `uploaded-image-${Date.now()}.webp`,
            alt: img.alt || ""
          })) || []
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="brand" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} placeholder="Enter brand name" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
          ...
          <CardContent className="space-y-4">
            <FormField control={form.control} name="ingredients" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea {...field} rows={2} placeholder="List ingredients" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="allergens" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} placeholder="e.g. Peanuts, Milk, Soy" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="shelfLife" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input {...field} placeholder="e.g. 12 months" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
          ...
          <CardContent className="space-y-4">
            {fields.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                No variants added. Click &quot;Add Variant&quot; to create a product variant.
              </div>
            )}
            ...
            {fields.map((field, index) => {
              ...
              return (
                <Card key={field.id} className={`border ${isDefault ? 'border-primary' : ''}`}>
                  <CardHeader 
                  ...
                  {isExpanded && (
                    <CardContent className="space-y-4 pt-0">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <FormField control={form.control} name={`variants.${index}.name`} render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input {...field} placeholder="e.g. 500g Pack" /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )} />
                        <FormField control={form.control} name={`variants.${index}.sku`} render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl><Input {...field} placeholder="Unique SKU" /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )} />
                        <FormField control={form.control} name={`variants.${index}.packageSize`} render={({ field }) => (
                          <FormItem>
                            <FormLabel>Package Size *</FormLabel>
                            <FormControl><Input {...field} placeholder="e.g. 500g, 1L" /></FormControl>
                            <FormMessage />
                          </FormItem>
                        )} />
                      </div>
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        <FormField control={form.control} name={`variants.${index}.mrp`} render={({ field }) => (
                          <FormItem>
                        ...
                        <FormField control={form.control} name={`variants.${index}.discountSource`} render={({ field }) => (
                          <FormItem>
                      ...
                      <div className="grid grid-cols-5 gap-4">
                        ...
                        <FormField control={form.control} name={`variants.${index}.breadth`} render={({ field }) => (
                          <FormItem>
                        ...
                        <FormField control={form.control} name={`variants.${index}.weightUnit`} render={({ field }) => (
                          <FormItem>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <FormField control={form.control} name={`variants.${index}.availabilityStatus`} render={({ field }) => (
                          <FormItem>
                        ...
                        <FormField control={form.control} name={`variants.${index}.isActive`} render={({ field }) => (
                      ...
                      <div className="space-y-2">
                        ...
                        <VariantImageUpload 
                          index={index} 
                          form={form}
                          initialImages={initialData?.variants?.[index]?.images || []}
                        />
                      </div>
                    </CardContent>
                  )}
...
function VariantImageUpload({ index, form, initialImages }: VariantImageUploadProps) {
  const [images, setImages] = useState<Array<{ file: File | null; alt: string; preview: string; finalUrl?: string }>>(
  ...
  const handleImagesChange = useCallback((newImages: Array<{ file: File | null; alt: string; preview: string; finalUrl?: string }>) => {
    setImages(newImages)
    form.setValue(`variants.${index}.images`, newImages.map(img => ({
      file: img.file,
      alt: img.alt,
      preview: img.preview,
    ...
    if (newImages.length > 0 && (newImages[0].finalUrl || newImages[0].preview)) {
      form.setValue(`variants.${index}.thumbnailUrl`, newImages[0].finalUrl || newImages[0].preview)
    }
  }, [form, index])
