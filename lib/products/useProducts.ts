...
export const useProductsByCategory = (categoryId: string, pagination: PaginationInput = { page: 1, limit: 10 }) => {
...
export const useSearchProducts = (searchTerm: string, pagination: PaginationInput = { page: 1, limit: 10 }) => {
...
export const useCreateProduct = () => {
  const [mutate, { loading, error }] = useMutation(CREATE_PRODUCT, {
    refetchQueries: [
      { query: GET_PRODUCTS, variables: { pagination: { page: 1, limit: 10 }, includeOutOfStock: false } }
    ],
    onError: (error: any) => {
...
export const useUnarchiveProduct = () => {
  const [mutate, { loading, error }] = useMutation(UNARCHIVE_PRODUCT, {
    refetchQueries: [
      { query: GET_PRODUCTS, variables: { pagination: { page: 1, limit: 10 }, includeOutOfStock: true } }
    ],
    onError: (error: any) => {
